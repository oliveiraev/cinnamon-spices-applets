...
MyApplet.prototype = {
  ...
  _init: function(metadata, orientation, panel_height, instance_id) {
    ...
    this.bytes_start_time =
      AppletConstants.BytesStartTime.START_OF_CURRENT_SESSION;
  ...
  _init_scripts: function() {
    try {
      ...
      if (error.length > 0) {
        global.log(
          uuid +
            " error while granting executable permission to scripts: " +
            error
        );
      }
    } catch (exception) {
  ...
  _grant_executable_permission: function(directory_path) {
    let process = new ShellUtils.ShellOutputProcess([
      "chmod",
      "-R",
      "755",
      directory_path
  ...
  _connect_signals: function() {
    ...
    global.connect(
      "scale-changed",
      Lang.bind(this, this.on_panel_height_changed)
    );
  },
  ...
  _init_checked_interfaces: function() {
    ...
    let checked_interfaces_string = this.settings.getValue(
      "checked_interfaces_string"
    );
    let network_interfaces = checked_interfaces_string.split(
      this.settings_separator
    );
    for (let network_interface of network_interfaces) {
      let is_valid = this.array_contains(
        this.network_interfaces,
        network_interface
      );
      if (is_valid) {
  ...
  _init_list_connections_process: function() {
    this.list_connections_process = new ShellUtils.TerminalProcess(
      this.list_connections_command
    );
  ...
  _init_menu_item_network: function() {
    this.menu_item_network = new AppletGui.CheckboxMenuItem(
      _("Network interfaces")
    );
    this.menu_item_network.set_callback_option_toggled(
      this,
      this.on_menu_item_network_toggled
    );
    this._reload_menu_item_network();
    this._applet_context_menu.addMenuItem(this.menu_item_network);
  },

  on_menu_item_network_toggled: function(
    option_index,
    option_name,
    is_checked
  ) {
  ...
  check_network_interface: function(network_interface) {
    let exists = this.array_contains(
      this.checked_interfaces,
      network_interface
    );
    if (!exists) {
  ...
  update_checked_interfaces_string: function() {
    this.checked_interfaces_string = this.checked_interfaces.join(
      this.settings_separator
    );
  },
  ...
  _init_menu_item_byte_start_times: function() {
    ...
    this.menu_item_byte_start_times = new AppletGui.RadioMenuItem(
      _("Total data start"),
      start_times
    );
    this._update_menu_item_byte_start_times_option();
    this.menu_item_byte_start_times.set_callback_option_clicked(
      this,
      this.on_menu_item_byte_start_times_clicked
    );
  ...
  _init_menu_item_gui: function() {
    this.menu_item_gui = new AppletGui.RadioMenuItem(_("Gui"), [
      _("Compact"),
      _("Large")
    ]);
    this.menu_item_gui.set_active_option(this.gui_speed_type);
    this.menu_item_gui.set_callback_option_clicked(
      this,
      this.on_menu_item_gui_clicked
    );
    this._applet_context_menu.addMenuItem(this.menu_item_gui);
    this._applet_context_menu.connect(
      "open-state-changed",
      Lang.bind(this, this.on_context_menu_state_changed)
    );
  },
  ...
  _init_hover_popup: function() {
    this.hover_popup = new AppletGui.HoverMenuTotalBytes(
      this,
      this.orientation,
      this.gui_value_order
    );
    this.on_hover_popup_css_changed();
  ...
  _init_gui: function() {
    this.gui_speed = new AppletGui.GuiSpeed(
      this._panelHeight,
      this.gui_speed_type,
      this.gui_value_order,
      this.decimal_places
    );
    this.gui_data_limit = new AppletGui.GuiDataLimit(
      this._panelHeight,
      this.gui_data_limit_type
    );
  ...
  _run_calculate_speed: function() {
    ...
    Mainloop.timeout_add(
      ...
      Lang.bind(this, this._run_calculate_speed_running)
    );
  },
  ...
  _calculate_speed: function() {
    try {
      ...
      let received_total = this.convert_to_two_decimals_string(
      ...
      global.logError(
        uuid + " error while calculating download and upload speed: " + e
      );
    }
  },
  ...
  scale: function(bytes) {
    return this.display_mode == AppletConstants.DisplayMode.SPEED
      ? Math.round(bytes / this.update_every)
      : bytes;
  },
  ...
  round_output_number: function(number) {
    let output_number =
      this.decimal_places == AppletConstants.DecimalPlaces.AUTO
        ? this.round_output_number_auto(number)
        : number.toFixed(this.decimal_places);
  ...
  update_gui_data_limit: function(bytes_received_total, bytes_sent_total) {
    ...
    let percentage = this.is_zero(this.data_limit)
      ? 0
      : bytes_total / this.convert_data_limit_bytes();
  ...
  invoke_data_limit_command_if_exceeded: function() {
    if (
      !this.data_limit_command_invoked &&
      this.data_limit_command_enabled &&
      this.data_limit_exceeded()
    ) {
  ...
  invoke_data_limit_command: function() {
    let terminal_process = new ShellUtils.TerminalProcess(
      this.data_limit_command
    );
  ...
  _run_write_bytes: function() {
    if (this.write_every > 0) {
      this._write_bytes_total();
      Mainloop.timeout_add(
        this.write_every * 1000,
        Lang.bind(this, this._run_write_bytes_running)
      );
    } else {
      Mainloop.timeout_add(
        1000,

...
class IconMenuItem extends PopupMenu.PopupBaseMenuItem {
  constructor(text, icon) {
    ...
    let label = new St.Label({
      style_class: "xCenter-menuItemLabel",
      text: text
    });
...
class VolumeMenuItem extends IconMenuItem {
  constructor(volume, mounted) {
    ...
    super(
      volume.get_name(),
      St.TextureCache.get_default().load_gicon(null, icon, menu_item_icon_size)
    );
...
class PlaceMenuItem extends FolderTypeMenuItem {
  constructor(uri, text, iconName) {
    ...
    if (iconName && icon_exists(iconName)) {
      ...
      icon = St.TextureCache.get_default().load_gicon(
        null,
        fileInfo.get_icon(),
        menu_item_icon_size
      );
...
class MyApplet extends Applet.TextIconApplet {
  constructor(metadata, orientation, panel_height, instanceId) {
    try {
      ...
      this.recentManager.connect(
        "changed",
        Lang.bind(this, this.buildRecentDocumentsSection)
      );
      Main.placesManager.connect(
        "bookmarks-updated",
        Lang.bind(this, this.buildUserSection)
      );
      this.volumeMonitor = Gio.VolumeMonitor.get();
      this.volumeMonitor.connect(
        "volume-added",
        Lang.bind(this, this.updateVolumes)
      );
      this.volumeMonitor.connect(
        "volume-removed",
        Lang.bind(this, this.updateVolumes)
  ...
  bindSettings() {
    ...
    if (recentSizeLimit % 5 !== 0)
      this.recentSizeLimit = Math.ceil(recentSizeLimit / 5) * 5;
    this.setKeybinding();
  }
  ...
  buildMenu() {
    try {
      ...
      let userTitle = new PopupMenu.PopupBaseMenuItem({
        style_class: "xCenter-title",
        reactive: false
      });
      userPane.addMenuItem(userTitle);
      userTitle.addActor(
        new St.Label({ text: GLib.get_user_name().toUpperCase() })
      );
      ...
      let systemTitle = new PopupMenu.PopupBaseMenuItem({
        style_class: "xCenter-title",
        reactive: false
      });
      ...
      let systemSearchImage = new St.Icon({
        icon_name: "edit-find",
        icon_size: 10,
        icon_type: St.IconType.SYMBOLIC
      });
      ...
      let systemScrollBox = new St.ScrollView({
        style_class: "xCenter-scrollBox",
        x_fill: true,
        y_fill: false,
        y_align: St.Align.START
      ...
      if (this.showRecentDocuments) {
        ...
        let recentTitle = new PopupMenu.PopupMenuItem(_("RECENT DOCUMENTS"), {
          style_class: "xCenter-title",
          reactive: false
        });
        recentPane.addMenuItem(recentTitle);

        let recentScrollBox = new St.ScrollView({
          style_class: "xCenter-scrollBox",
          x_fill: true,
          y_fill: false,
          y_align: St.Align.START
        ...
        recentPane._connectSubMenuSignals(
          this.recentSection,
          this.recentSection
        );
  ...
  buildUserSection() {
    ...
    for (let bookmark of bookmarks) {
      let bookmarkItem = new FolderTypeMenuItem(
        bookmark.name,
        bookmark.iconFactory(menu_item_icon_size)
      );
      this.userSection.addMenuItem(bookmarkItem);
      let launch = bookmark.launch;
      bookmarkItem.connect(
        "activate",
        Lang.bind(this, function() {
          launch();
        })
  ...
  buildSystemSection() {
    ...
    if (this.showComputer) {
      let computer = new PlaceMenuItem(
        "computer:///",
        _("Computer"),
        "computer"
      );
    ...
    //file system
    if (this.showRoot) {
      let fileSystem = new PlaceMenuItem(
        "file:///",
        _("File System"),
        "drive-harddisk"
      );
    ...
    if (this.showNetwork) {
      ...
      let connectToItem = new IconMenuItem(
        bookmark.name,
        bookmark.iconFactory(menu_item_icon_size)
      );
      this.systemSection.addMenuItem(connectToItem);
      connectToItem.connect(
        "activate",
        Lang.bind(this, function() {
          bookmark.launch();
        })
  ...
  buildRecentDocumentsSection() {
    ...
    let appOpeningFolders = Gio.app_info_get_default_for_type(
      "inode/directory",
      false
    ).get_executable(); // usually returns: nemo

    let showCount;
    if (this.recentSizeLimit == 0) showCount = recentDocuments.length;
    else
      showCount =
        this.recentSizeLimit < recentDocuments.length
          ? this.recentSizeLimit
    ...
    while (i < showCount) {
      ...
      let file = Gio.file_new_for_path(
        "%s".format(recentInfo.get_uri_display())
      );
      if (file.query_exists(null)) {
        ...
        let default_for_type = Gio.app_info_get_default_for_type(
          recentInfo.get_mime_type(),
          false
        );
        if (default_for_type) {
          ...
          if (
            mimeType.substr(0, 6) === "image-" &&
            mimeType.substr(0, 8) !== "image-x-" &&
            !icon_exists(mimeType)
          ) {
        ...
        if (gicon) {
          recentItem = new RecentFileMenuItem(
  ...
  buildTrashItem() {
    ...
    if (!this.trashMonitor) {
      ...
      this.trashMonitor.connect(
        "changed",
        Lang.bind(this, this.buildTrashItem)
      );
    }
  ...
  setPanelIcon() {
    if (
      ...
      if (this.panelIcon.search("-symbolic.svg") == -1)
        this.set_applet_icon_path(this.panelIcon);
      else this.set_applet_icon_symbolic_path(this.panelIcon);
    } else if (icon_exists(this.panelIcon)) {
      if (this.panelIcon.search("-symbolic") != -1)
        this.set_applet_icon_symbolic_name(this.panelIcon);

...
MyApplet.prototype = {
  ...
  _bind_settings: function() {
    for (let [binding, property_name, callback] of [
      ...
      [
        Settings.BindingDirection.IN,
        "terminal_command",
        this.on_terminal_command_changed
      ],
      [
  ...
  get_column_names_insert: function() {
    ...
    let is_bytes_read_column = this.array_contains(
      this.column_indexes_insert,
      this.bytes_read_index
    );
    if (is_bytes_read_column) {
      column_names_insert.push(this.total_bytes_read_column_name);
    }
    let is_bytes_written_column = this.array_contains(
      this.column_indexes_insert,
      this.bytes_written_index
    );
  ...
  read_string: function(file, default_string) {
    try {
      ...
      let string =
        array_bytes.length > 0
          ? array_bytes.toString()
  ...
  read_sectors_size: function(disk_name) {
    ...
    let string = file_sectors_size.exists()
      ? this.read_number(file_sectors_size, this.default_sector_size)
  ...
  _init_terminal_process: function() {
    this.terminal_process = new ShellUtils.TerminalProcess(
      this.terminal_command
    );
  ...
  _init_menu_item_gui: function() {
    this.menu_item_gui = new AppletGui.RadioMenuItem("Gui", [
      "Compact",
      "Large"
    ]);
  ...
  _init_menu_item_disks: function() {
    ...
    this.menu_item_disks.set_callback_option_clicked(
      this,
      this.on_menu_item_disks_clicked
    );
  ...
  update_input_output_info: function(line) {
    ...
    let info =
      device_name in this.input_output_infos
        ? this.input_output_infos[device_name]
        : new InputOutputInfo();
  ...
  update_input_output_info_values: function(info, values) {
    ...
    info.inputs_outputs_in_progress = parseInt(
      values[this.inputs_outputs_in_progress_index]
    );
    info.time_inputs_outputs_ms = parseInt(
      values[this.time_inputs_outputs_ms_index]
    );
    info.weighted_time_inputs_outputs_ms = parseInt(
      values[this.weighted_time_inputs_outputs_ms_index]
    );
  ...
  sort_columns: function(table) {
    ...
    table =
      ...
      this.column_index_sort == this.device_name_index
        ? table.sort(
            this.sort_string_rows(
              this.column_index_sort,
              this.column_sort_order
            )
          )
        : table.sort(
            this.sort_integer_rows(column_index, this.column_sort_order)
          );
    return table;
  },
  ...
  sort_string_rows: function(column_index, sort_order) {
    return function(row_values_1, row_values_2) {
      ...
      return sort_order == AppletConstants.SortOrder.ASCENDING
  ...
  round_byte_column_values: function(row_values) {
    for (let index of this.byte_indexes) {
      if (index < row_values.length) {
        row_values[index] = this.convert_byte_column_value_to_readable_string(
          row_values[index]
        );
      }
    }
  ...
  round_byte_column_number: function(number, unit) {
    let output_number =
      this.hover_popup_decimal_places == AppletConstants.DecimalPlaces.AUTO
        ? this.round_byte_column_auto(number, unit)
        : number.toFixed(this.hover_popup_decimal_places);
    return output_number;
  },

  round_byte_column_auto: function(number, unit) {
    let output_number = this.is_less_gigabyte(unit)
      ? number.toFixed(0)
      : number.toFixed(2);
    return output_number;
  },
    ...
    for (let i = 0; i < table.length; ++i) {
      for (let j = 0; j < columns; ++j) {
        let min_length =
          j == last_column_index
            ? max_lengths[j]
            : max_lengths[j] + this.column_spaces;
  ...
  get_sectors_read_iteration: function() {
    ...
    let sectors_read_iteration = this.calculate_bytes_difference(
      info.sectors_read,
      info.previous_sectors_read
    );
  ...
  get_sector_size: function() {
    let sector_size =
      this.disk_name in this.disk_sector_sizes
        ? this.disk_sector_sizes[this.disk_name]
        : this.default_sector_size;
    return sector_size;
  },
  ...
  round_output_number: function(number) {
    let output_number =
      this.gui_decimal_places == AppletConstants.DecimalPlaces.AUTO
        ...
        : number.toFixed(this.gui_decimal_places);
    return output_number;
  },
  ...
  get_sectors_written_iteration: function() {
    let info = this.input_output_infos[this.disk_name];
    let sectors_written_iteration = this.calculate_bytes_difference(

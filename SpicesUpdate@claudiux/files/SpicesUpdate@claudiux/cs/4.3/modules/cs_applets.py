...
class AppletsViewSidePage(SidePage):
    ...
    def __init__(self, content_box, module):
        ...
        super(AppletsViewSidePage, self).__init__(_("Applets"),
                                                  "cs-applets", _("applet"), content_box, module=module)
    ...
    def load(self, window):
        ...
        download_applets_page = DownloadSpicesPage(
...
class ManageAppletsPage(ManageSpicesPage):
    directories = [("%s/.local/share/cinnamon/applets") %
    ...
    def __init__(self, parent, spices, window):
        super(ManageAppletsPage, self).__init__(
        ...
        if len(sys.argv) > 2 and sys.argv[1] == "applets" and sys.argv[2][0:5] == "panel":
            ...
            self.panel_id = int(self.spices.settings.get_strv(
        ...
        self.spices.send_proxy_signal(
            'highlightPanel', '(ib)', self.panel_id, True)
        ...
        self.spices.settings.connect(
            'changed::panels-enabled', self.panels_changed)
        self.panels_changed()
    ...
    def previous_panel(self, *args):
        self.spices.send_proxy_signal(
            'highlightPanel', '(ib)', self.panel_id, False)

...
class Module:
    ...
    def make_group(self, group_label, widget, add_widget_to_size_group=True):
        self.size_groups = getattr(
            self,
            "size_groups",
            [SizeGroup.new(SizeGroupMode.HORIZONTAL) for x in range(2)],
        )
        box = SettingsWidget()
        label = Gtk.Label()
    ...
    def create_button_chooser(
        ...
        if self is None:
            return
        chooser = PictureChooserButton(
            num_cols=num_cols,
            button_picture_size=button_picture_size,
            menu_pictures_size=menu_pictures_size,
    ...
    def _load_cinnamon_themes(self):
        ...
        valid = walk_directories(
            dirs,
            lambda d: os.path.exists(os.path.join(d, "cinnamon")),
            return_directories=True,
        )
        valid.sort(key=lambda a: a[0].lower())
        res = []

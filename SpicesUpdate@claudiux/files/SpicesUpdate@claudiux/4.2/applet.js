...
function notify_send(
  ...
  if (icon_path) {
    ...
    _icon = "%s/.local/share/cinnamon/applets/%s/icons/spices-update-symbolic.svg".format(
      HOME_DIR,
      UUID
    );
  }
  let notification = 'notify-send "%s" "%s" -i %s -t %s -u %s'.format(
    _("Spices Update"),
    message,
    _icon,
    duration.toString(),
...
class SpicesUpdate extends Applet.TextIconApplet {
        ...
        Lang.bind(this, function(self, action) {
          if (action == "spices-update") {
            Util.spawnCommandLine(
              "%s %s -t 1 -s date".format(CS_PATH, type.toString())
            );
          } else {
  ...
  */
      ...
      for (var i = 0; i < this.unprotected_applets.length; i++) {
        ...
        if (d.query_exists(null)) {
          ...
          if (
            ...
            let created = this._get_member_from_cache(
              type,
              a["name"],
              "created"
            );
  ...
  makeMenu() {
    ...
    for (let t of TYPES) {
      ...
      this.spicesMenuItems[t].connect("activate", event => {
        Util.spawnCommandLine(
          "%s %s -t 1 -s date".format(CS_PATH, t.toString())
        );
      });
    ...
    configure.connect("activate", event => {
      Util.spawnCommandLine(
        "cinnamon-settings applets %s %s".format(UUID, this.instanceId)
      );
    });
  ...
  icon_rotate() {
    ...
    let size = Math.round(
      this.getPanelIconSize(St.IconType.SYMBOLIC) / global.ui_scale
    );

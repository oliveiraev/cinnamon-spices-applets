...
class WeatherApplet extends Applet.TextIconApplet {
  ...
  async LoadJsonAsync(query) {
    let json = await new Promise((resolve, reject) => {
      ...
      this._httpSession.queue_message(message, (session, message) => {
        ...
        try {
          this.log.Debug("API full response: " +
                         message.response_body.data.toString());
          let payload = JSON.parse(message.response_body.data);
          resolve(payload);
        } catch (e) {
          this.log.Error("Error: API response is not JSON. The response: " +
  ...
  displayForecast() {
    try {
      for (let i = 0; i < this._forecast.length; i++) {
        ...
        let dayName = GetDayName(forecastData.date, this.currentLocale,
                                 this.weather.location.timeZone);
  ...
  wipeForecastData() { this.forecasts = []; };
  ...
  rebuildFutureWeatherUi() {
    ...
    for (let i = 0; i < this._forecastDays; i++) {
      ...
      let forecastBox = new St.BoxLayout({style_class : STYLE_FORECAST_BOX});
  ...
  unitToUnicode(unit) { return unit == "fahrenheit" ? '\u2109' : '\u2103'; }
  ...
  HandleError(error) {
    ...
    let nextRefresh = (this.errorCount > 0)

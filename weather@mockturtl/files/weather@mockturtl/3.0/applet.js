...
var WeatherApplet = (function(_super) {
          ...
          case 0:
            return [
              ...
              new Promise(function(resolve, reject) {
                ...
                _this._httpSession.queue_message(message, function(
                  session,
                  message
                ) {
                  if (!message)
      ...
      return __generator(this, function(_b) {
        switch (_b.label) {
          ...
          case 9:
            e_2 = _b.sent();
            this.log.Error(
              "Generic Error while refreshing Weather info: " + e_2
            );
            this.HandleError({
              type: "hard",
              detail: "unknown",
  ...
  WeatherApplet.prototype.ProcessWeatherData = function(
    weatherInfo,
    locationData
  ) {
    if (!!locationData) {
  ...
  WeatherApplet.prototype.displayWeather = function() {
    try {
      ...
      if (!!this.weather.extra_field) {
        this._currentWeatherApiUniqueCap.text = _(
          this.weather.extra_field.name
        );
        var value = void 0;
        switch (this.weather.extra_field.type) {
          ...
          case "temperature":
            value =
              TempToUserUnits(
                this.weather.extra_field.value,
                this._temperatureUnit
  ...
  WeatherApplet.prototype.displayForecast = function() {
    try {
      for (var i = 0; i < this._forecast.length; i++) {
        ...
        var t_low = TempToUserUnits(
          forecastData.temp_min,
          this._temperatureUnit
        );
        var t_high = TempToUserUnits(
  ...
  WeatherApplet.prototype.rebuildCurrentWeatherUi = function() {
    ...
    var middleColumn = new St.BoxLayout({
      vertical: true,
  ...
  WeatherApplet.prototype.HandleError = function(error) {
    ...
    var nextRefresh =
      this.errorCount > 0
        ...
        : this.LOOP_INTERVAL;
    this.log.Error(
      "Retrying in the next " + nextRefresh.toString() + " seconds..."
    );
  };
  WeatherApplet.prototype.HandleHTTPError = function(
    service,
    error,
    ctx,
    callback
